...
<body>
  <div class="p-3">
    <h1>Albums</h1>
    <table class="table table-hover" id="tableAlbum">
      <thead>
        <th>ID</th>
        <th>Nombre</th>
        <th>Autor</th>
...
$(document).ready( function () {
  var table = $('#tableAlbum').DataTable(
    {
      "ajax":'AlbumController/albumsjs',
      "error": function(d){
  ...
  editarAlbum('#tableAlbum tbody', table);
  eliminarAlbum('#tableAlbum tbody', table);
} );

//Funcion para editar un genero
function editarAlbum(tbody, table){
  $(tbody).on("click", "button.editar", function(){
    var data = table.row($(this).parents("tr")).data();
    var idAlbum = data.id;
    var pageEditAlbum = 'AlbumController/edit/'+idAlbum+'';
    document.location.href=pageEditAlbum;
  });
}

//Funcion para eliminar al usuario
function eliminarAlbum(tbody, table){
  $(tbody).on("click", "button.eliminar", function(){
    var data = table.row($(this).parents("tr")).data();
    var idAlbum = data.id;
    var nameAlbum = data.name;

    Swal.fire({
      title: '¿Estas seguro de eliminar '+nameAlbum+'?',
      showDenyButton: true,
      ...
      if (result.isDenied) {
        document.location.href='AlbumController/delete/'+idAlbum+'';
      }
    })
